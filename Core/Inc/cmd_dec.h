...
////extern void UART_Send(const unsigned char *p, int l, EResultOut out);
extern void Generic_Send(char *s, int l, EResultOut out);
...
#define print_log(out, ...)		do {\
									if (out == UART_OUT) {\
										...
										UART_Send((uint8_t *)XPrintBufUART, strlen((char *)XPrintBufUART), VCP_OUT);\
									}\
									if (out == VCP_OUT) {\
										snprintf((char *)XPrintBufVCP, XPRINT_LEN, __VA_ARGS__);\
										UART_Send((uint8_t *)XPrintBufVCP, strlen((char *)XPrintBufVCP), VCP_OUT);\
									}\
								} while(0)
