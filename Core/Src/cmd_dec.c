...
/* ---------------------------------------------- */
void Generic_Send(char *s, int l, EResultOut out)
{
	if (out == UART_OUT)
	{
		UART_Send((unsigned char *)s, l, out);
	}
	if (out == VCP_OUT)
	{
		UART_Send((unsigned char *)s, l, out);
	...
	do
	{
		...
		if (cmdRes->cmd)
			if (*cmdRes->cmd != '\0')	/* ignore empty line, just spaces */
			{
				...
				{
					Generic_Send("*E: unknown >", 13, out);
					Generic_Send(cmdRes->cmd, cmdRes->cmdLen, out);
					Generic_Send("<\r\n", 3, out);
...
{
	if (res->str)
		Generic_Send(res->str, CMD_lineLen(res->str), out);
	if (res->opt)
	{
...
ECMD_DEC_Status CMD_qspiclk(TCMD_DEC_Results *res, EResultOut out)
{
	if ( ! res->val[0])
	{
		print_log(out, "QSPI div: %d\r\n", (int)gCFGparams.QSPIdiv);
		return CMD_DEC_OK;
	}
